...
def create_employee():
    ...
    try:
        ...
        employee = Employee(
            ...
            barcode=request.form.get('barcode') or f"EMP-BAR-{employee_id}",
            monthly_salary=float(request.form.get('monthly_salary', 0.0) or 0.0),
...
def update_employee(id):
    ...
    try:
        ...
        if request.form.get('barcode'):
            employee.barcode = request.form.get('barcode')
        if request.form.get('monthly_salary') is not None:
            try:
                employee.monthly_salary = float(request.form.get('monthly_salary') or 0.0)
            except:
                pass
